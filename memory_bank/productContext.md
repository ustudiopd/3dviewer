...
2. 서버에서 `get_demo_with_model` RPC 함수로 데모와 모델 정보를 함께 조회
3. 데모 활성화 상태 및 만료일 확인
4. 모델 파일의 Signed URL 생성 (1시간 유효)
5. `DynamicModelViewer` 컴포넌트가 모델 로드
6. `@google/model-viewer`가 3D 모델 렌더링
7. 사용자가 마우스/터치로 모델 조작:
   - 드래그: 모델 회전
   - 휠: 확대/축소
   - 우클릭 + 드래그: 카메라 이동
8. 접속 통계 업데이트 (RPC 함수 사용)
9. 접속 로그 자동 수집 (API 호출)
...
3. 파일 선택 (GLB/GLTF, 최대 5GB)
4. 파일명 입력 (선택사항)
5. Draco 압축 및 KTX2 옵션 선택
6. 업로드 시작:
   - 파일을 Supabase Storage에 업로드 (`glb-models-private` 버킷)
   - `insert_model` RPC 함수를 통해 파일 메타데이터를 `models` 테이블에 저장
...
4. 만료일 설정 (선택사항)
5. 생성 버튼 클릭:
   - 8자리 접근 코드 자동 생성 (고유성 확인)
   - `insert_demo` RPC 함수를 통해 `demos` 테이블에 레코드 생성
   - 모델 ID와 접근 코드 연결
6. 생성된 데모가 목록에 표시
